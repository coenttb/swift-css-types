...
struct AnimationRangeTests {
    ...
    func testStartEndRange() {
        ...
        #expect(AnimationRange.startEnd(.namedRange(.entry), .percentage(50)).description == "entry 50%")
        #expect(AnimationRange.startEnd(.namedRange(.entry, 10), .namedRange(.contain, 75)).description == "entry 10% contain 75%")
    }
